    ...
    def create_simulator(system_type: str, room_params: Dict[str, Any], hvac_params: Dict[str, Any]):
        ...
        if system_type == "split-system":
            ...
            hvac = ChilledWaterHVACParameters(
                ...
                primary_secondary_loop=bool(hvac_params.get('primary_secondary_loop', True)),
                glycol_percentage=float(hvac_params.get('glycol_percentage', 0)),
                heat_exchanger_efficiency=float(hvac_params.get('heat_exchanger_efficiency', 0.85))
            )
